package hybrid_ga_tabu;

import genetic_algorithm.Gene;
import hybrid_ica_tabu.HybridTabuSearch;
...
public class HybridGA {
	...
	public HybridGA(int queens, int popSize, double crossRate,
	...
	public void run() {
		...
		if (best.getFitness() > 0) {
			HybridTabuSearch ts = new HybridTabuSearch(best.getQueens(), 300,
					25, 0);
			ts.run();
			best = new Gene(ts.getSolution(), random);
			System.out.println("Fitness: " + best.getFitness());
			tabuCount = ts.getCount();
		}
		BufferedWriter bw = null;
		FileWriter fw = null;
		String FILENAME = "Outputs/HybridGAOutput";
		int t = 1;
		try {
			...
			bw.write("Generations on GA: " + (count));
			bw.newLine();
